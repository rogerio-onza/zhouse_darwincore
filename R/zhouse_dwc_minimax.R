...
coalesce_columns <- function(df, target, candidates) {
  ...
  if (length(keep) > 1) {
    for (k in keep[-1]) out <- dplyr::coalesce(out, as.character(df[[k]]))
  }
  df[[target]] <- out
  df
}
...
split_semicolon <- function(x) {
  if (!is_non_empty(x)) return(character(0))
...
pt_title_case <- function(x) {
  if (!is_non_empty(x)) return(NA_character_)
  ...
  words <- map_chr(seq_along(words), function(i) {
    w <- words[[i]]
    if (i > 1 && w %in% small_words) return(w)
    str_to_title(w)
  })
...
normalize_group <- function(x) {
  if (!is_non_empty(x)) return("unknown")
  x_norm <- norm_key(x)
  if (str_detect(x_norm, "fung"))           return("fungi")
  if (str_detect(x_norm, "flora|planta"))   return("planta")
  "fauna"
}
...
has_uncertainty_marker <- function(x) {
  if (!is_non_empty(x)) return(FALSE)
  x_norm <- norm_key(x)
...
# Returns canonical name stripped of uncertainty markers.
canonical_name <- function(x) {
  ...
  genus  <- tokens[[1]]
  if (length(tokens) == 1) return(genus)

  second <- tokens[[2]]
  if (str_detect(str_to_lower(second), "^sp\\.?\\d*$|^spp\\.?$")) return(genus)
  if (!is_lower_taxon_token(second))                                return(genus)
  ...
  if (length(tokens) >= 4 && str_to_lower(tokens[[3]]) %in% c("subsp.", "ssp.", "var.", "f.", "forma")) {
    ...
    if (is_lower_taxon_token(tokens[[4]])) return(paste(genus, second, marker, tokens[[4]]))
    return(paste(genus, second))
  }

  if (length(tokens) >= 3 && is_lower_taxon_token(tokens[[3]])) return(paste(genus, second, tokens[[3]]))

  paste(genus, second)
}

# Returns canonical name, falling back to genus only when uncertain markers present.
safe_rank_name <- function(x) {
  can <- canonical_name(x)
  if (!is_non_empty(can)) return(NA_character_)
...
parse_rank_from_name <- function(x) {
  if (!is_non_empty(x)) {
    return(list(
      taxonRank = NA_character_, genus = NA_character_,
      specificEpithet = NA_character_, infraspecificEpithet = NA_character_
    ))
  }
  ...
  if (length(tokens) >= 4 && str_to_lower(tokens[[3]]) %in% c("subsp.", "ssp.", "var.", "f.")) {
    ...
    return(list(
      taxonRank = rank, genus = genus,
      specificEpithet = tokens[[2]], infraspecificEpithet = tokens[[4]]
    ))
  }

  if (length(tokens) >= 3 && is_lower_taxon_token(tokens[[3]])) {
    return(list(
      taxonRank = "subspecies", genus = genus,
      specificEpithet = tokens[[2]], infraspecificEpithet = tokens[[3]]
    ))
  }

  list(
    taxonRank = "species", genus = genus,
    specificEpithet = tokens[[2]], infraspecificEpithet = NA_character_
  )
}

status_rank <- function(x) {
  map <- c(
    "EX" = 8, "EW" = 7, "CR (PEX)" = 6, "CR" = 5,
    "EN" = 4, "VU" = 3, "NT" = 2, "LC" = 1, "NA" = 0
  )
...
pick_most_threatened <- function(x) {
  ...
  if (length(x) == 0) return(NA_character_)
  rk <- status_rank(x)
  if (all(is.na(rk))) return(NA_character_)
  x[[which.max(rk)]]
}
...
# Reads and parses the species criteria markdown table.
parse_criteria_md <- function(path) {
  if (!file.exists(path)) return(tibble(species = character(0), category = character(0)))

  lines     <- readLines(path, warn = FALSE, encoding = "UTF-8")
  tbl_lines <- lines[str_detect(lines, "^\\|")]

  if (length(tbl_lines) == 0) return(tibble(species = character(0), category = character(0)))
  ...
  out <- map_dfr(tbl_lines, function(line) {
    ...
    if (length(cat_idx) == 0) return(NULL)
    idx <- cat_idx[[length(cat_idx)]]
    if (idx <= 1) return(NULL)
    species <- parts[[idx - 1]]
    if (!is_non_empty(species)) return(NULL)
    if (str_detect(norm_key(species), "familia|especie|anexo|categoria")) return(NULL)
...
get_iucn_criteria <- function(genus, species, infra, key) {
  if (!is_non_empty(genus) || !is_non_empty(species) || !is_non_empty(key)) return(NA_character_)
...
run_florabr_check <- function(name_vec, kingdom, source_data) {
  ...
  if (length(name_vec) == 0) return(tibble())
  out <- tryCatch(
    florabr::check_names(
      data              = source_data,
      species           = name_vec,
...
# taxadb / faunabr validation for non-flora
...
if (nrow(nonflora_idx) > 0) {
  ...
  if (!is.null(gbif_db)) {
    taxadb_raw <- map_dfr(unique(nonflora_idx$name_query), function(nm) {
      ...
      if (!is.data.frame(out) || nrow(out) == 0) return(tibble(input_name = nm))
    ...
    if (length(ids_to_resolve) > 0) {
      acc_rows <- map_dfr(ids_to_resolve, function(idv) {
        ...
        if (!is.data.frame(out) || nrow(out) == 0) return(tibble(req_id = idv))
      ...
      taxadb_best <- taxadb_best %>%
        ...
        mutate(
          ...
          scientificNameAuthorship = if_else(use_acc & is_non_empty(scientificNameAuthorship_acc), scientificNameAuthorship_acc, scientificNameAuthorship),
          taxonomicStatus          = if_else(use_acc & is_non_empty(taxonomicStatus_acc),          taxonomicStatus_acc,          taxonomicStatus),
          ...
          specificEpithet          = if_else(use_acc & is_non_empty(specificEpithet_acc),          specificEpithet_acc,          specificEpithet),
          infraspecificEpithet     = if_else(use_acc & is_non_empty(infraspecificEpithet_acc),     infraspecificEpithet_acc,     infraspecificEpithet)
        ) %>%
...
nonflora_decisions <- nonflora_idx %>%
  ...
  left_join(
    taxadb_lookup %>% rename_with(~ paste0(.x, "_tx"), -match_key),
    by = c("final_key" = "match_key")
  ) %>%
  left_join(
    fauna_lookup %>% rename_with(~ paste0(.x, "_fa"), -match_key),
    by = c("final_key" = "match_key")
  ) %>%
  mutate(
...
# Parse rank components from resolved scientific names
...
# Apply criteria and conservation status
...
# IUCN criteria lookup

iucn_key    <- "YOUR_API_KEY"
...
# Remove duplicate columns that can arise from joins
cols_to_remove <- names(base_final)[str_detect(names(base_final), "\\.(x|y)$")]
if (length(cols_to_remove) > 0) base_final <- base_final %>% select(-all_of(cols_to_remove))
...
if (!"establishmentMeans" %in% template_cols) {
  ...
  template_cols <- if (is.na(insert_at)) {
    c(template_cols, "establishmentMeans")
  } else {
    append(template_cols, "establishmentMeans", after = insert_at)
  }
}

for (col in template_cols) {
  if (!col %in% names(dwc)) dwc[[col]] <- NA_character_
}
...
name_examples <- base %>%
  ...
  summarise(originalName = combine_pipe(name_clean), .groups = "drop")
...
writexl::write_xlsx(
  list(auditoria = audit_tbl, nao_resolvidos = audit_unresolved),
  audit_output_path
)

message("Process finished.")
message("DwC output:   ", output_path)
message("Audit output: ", audit_output_path)
